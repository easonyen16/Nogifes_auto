...
::檔案重新命名
if %cnt% LSS 1000 (rename %~dp0\auto\*.m2v reward_movie_00%cnt%.264_med.m2v)^
else (rename %~dp0\auto\*.m2v reward_movie_0%cnt%.264_med.m2v)
if %cnt% LSS 1000 (rename %~dp0\auto\*.wav reward_movie_00%cnt%.avi.wav)^
else (rename %~dp0\auto\*.wav reward_movie_0%cnt%.avi.wav)
::影片合併
